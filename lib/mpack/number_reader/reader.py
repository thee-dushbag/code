...
class NumberReader:
    ...
    def read(self, number: float, ignore_pz: bool = True) -> str:
        ...
        namedgroups = [self.locale.group(group, index) for index, group in enumerate(grpnames)]
        namedgroups = list(reversed(namedgroups))
        dname = '' if dnum == '0' and ignore_pz else self.locale.read_decimal_number(dnum)
