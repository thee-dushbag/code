from typing import Callable, Any, Coroutine, Any, Awaitable, Mapping, cast
from aiohttp_mako import render_template, template as _template
...
def get_url(request: web.Request):
    def url(name: str, **kwargs):
        return request.app.router[name].url_for(**{k: str(v) for k, v in kwargs.items()})
    return url
...
def page_template(page_template_file: str, *, page_key: str = 'page', data_key: str = 'data', global_key: str = 'g'):
    def get_template(page_name: str, **global_data):
        def wrapper(handler: Callable):
            async def get_data(req: web.Request):
                ...
                data = {global_key:gbl_data, page_key:page_name, data_key:MyDict(**u_data)}
                res = render_template(page_template_file, req, data)
                if res.text: res.text = html_minify(res.text)
                return res
            return get_data
        return wrapper
    return get_template
...
def template(page_name: str) -> Callable[..., Handler]:
    def _handler(func: _ViewHandler) -> Handler:
        async def _get_resp(req: web.Request) -> web.Response:
            ...
            if resp.text: resp.text = html_minify(resp.text)
            return resp
        return _get_resp
    return _handler
