...
class Resourse:
    ...
    permission: Permission = attrs.field(default=Permission.PROTECTED, converter=_permission_int_converter)
    owner: int
    resource: str
...
def _file_db_helper(file: Path | str) -> Path:
    ...
    assert f.is_file(), \
        f'database file is not a file: {f!r}'
    return f
