        ...
        def _inner_two(*args, **kwargs):
            ...
            class Wrapper:
                def __getattr__(self, _name: str) -> Any:
                    if predicate(_name):
                        return getattr(_wrapped, _name)
                    raise Exception(
                        f"ReadError: Access to attribute {_name!r} outside class {Class.__name__}"
                    )

                def __setattr__(self, _name: str, _value: Any) -> None:
                    if predicate(_name):
                        return setattr(_wrapped, _name, _value)
                    raise Exception(
                        f"WriteError: Access to attribute {_name!r} outside class {Class.__name__}"
                    )
