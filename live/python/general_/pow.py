...
def time_basic_vs_int_exp():
    ...
    def run_test(power) -> tuple[float, float]:
        return average(
            [power(i, j) for i in tqdm(range(1, 1000, 2)) for j in range(1, 500)]
        )
    ...
    print(
        f"PowerFloat took a total of {power_pow_total} total and {power_pow_avg} average seconds."
    )
    print(
        f"IntExpPow took a total of {iexp_total} total and {iexp_avg} average seconds."
    )
    print(
        f"BasicPower took a total of {basic_total} total and {basic_avg} average seconds."
    )


# if __name__ == "__main__":
#     time_basic_vs_int_exp()
