...
def say_hi(name: str):
    ...
    console.print(
        f"[bold blue]Hello [italic][yellow]{name}[/italic][blue], how was your day[purple]?"
    )
...
@click.option("--delay", "-d", default=0.1, type=float)
def counter(n: int, delay: float, bar: bool):
    ...
    rng = (
        track(range(n), description="Counting...", console=console) if bar else range(n)
    )
    for i in rng:
