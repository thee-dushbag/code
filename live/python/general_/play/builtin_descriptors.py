...
class Property:
    ...
    def __get__(self, inst, cls):
        if self.fget: return self.fget(inst)
        raise TypeError(f'Cannot get this attribute.')

    def __set__(self, inst, value):
        if self.fset: return self.fset(inst, value)
        raise TypeError(f'Cannot set this attribute.')

    def __delete__(self, inst):
        if self.fdel: return self.fdel(inst)
...
class Test:
    ...
    def emailGetter(self): return self._email
    def emailSetter(self, value): self._email = value
    def emailDeleter(self): self._email = None
