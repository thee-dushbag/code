...
async def get_todos(req: web.Request):
    ...
    if not offset.isnumeric(): raise _e
    if not limit.isnumeric(): raise _e
    offset, limit = map(lambda i: abs(int(i)), (offset, limit))
    end = limit + offset
    ltodos = tuple(asdict(t) for i,t in enumerate(todos)
...
async def delete_todo(req: web.Request):
    ...
    def get_target(todo: Todo):
        if not todo.todo_id == tid: return 1
        nonlocal target
        target = todo
    global todos
    todos = list(filter(get_target, todos))
    if target is None: raise _e
...
async def edit_todo(req: web.Request):
    ...
    def get_target(todo: Todo):
        if todo.todo_id == tid:
            nonlocal target
            target = todo
    for todo in todos: get_target(todo)
    if target is None: raise _e
    data = await req.post()
    if 'title' in data: target.title = data['title']
    if 'desc' in data: target.desc = data['desc']
    return web.json_response(asdict(target))
