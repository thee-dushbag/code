...
import basicauth as bs, json
from utils import DatabaseStore, UserData, DB_STORE_KEY, CHECK_AUTH_KEY
...
def ensure_valid_user(data, state=None):
    ...
    for prop in [name, passwd]:
        if not prop in data:
            raise web.HTTPUnauthorized(reason=f"Cannot {state} Account: {prop} not provided")
        if not data.get(prop):
            raise web.HTTPUnauthorized(reason=f"Cannot {state} Account: {prop} has invalid value")
...
class UserView(web.View):
    def __init__(self, request: web.Request) -> None:
        ...
        check_auth: Callable[[web.Request], tuple[str, str]] = self.request.app[CHECK_AUTH_KEY]
