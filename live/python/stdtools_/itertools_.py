...
def callback_function(iterable: Iterable):
    def _impl(callback: IteratorCallback, accumulator: Callable[[Optional[Any], Optional[Any]], Any]):
        initial = accumulator()
...
def itertools_():
    ...
    adj = _format("Always_Iterable: {} : {}", data, tuple(mit.always_iterable(data, None)))
    adj = _format("Always_Reversible: {} : {}", data, tuple(mit.always_reversible(data)))
    data = ''.join(str(i) for i in range(1, 10)), 3
    adj = _format("batched: {} : {}", data[0], tuple(mit.batched(data[0], data[1])))
    data = range(10), 6
    adj = _format("before_and_after: {} : {}", data, tuple(tuple(x) for x in mit.before_and_after(is_less_than(data[1], True), data[0])))
    data = range(11)
    buckets = mit.bucket(data, e_or_o)
    adj = _format("bucket: {} : {}", data, {key: tuple(buckets[key]) for key in ('even', 'odd')})
    data = range(1, 11)
    with mit.callback_iter(partial(callback_function(data), accumulator=int_accumulator)) as accum:
